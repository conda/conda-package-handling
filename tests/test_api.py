...
def test_convert_keyerror(tmpdir, mocker):
    ...
    def create_file_and_raise(*args, **kwargs):
        out_fn = pathlib.Path(
            tmpdir, pathlib.Path(tarfile[: -len(".tar.bz2")] + ".conda").name
        )
        print("out fn", out_fn)
        out_fn.write_text("")
    ...
    mocker.patch(
        "conda_package_streaming.transmute.transmute", side_effect=create_file_and_raise
    )
...
def test_create_filelist(tmpdir, mocker):
    ...
    with pytest.raises(FileNotFoundError):
        api.create(
            str(tmpdir), "filelist-not-found.txt", str(tmpdir / "newconda.conda")
        )
